...
export const editTrailerHomePageUrlController = async (req, res) => {
  try {
    ...
    if (newFile) {
      ...
      const oldImageFileName = findById.url.split(`/`).pop().split('?alt=media')[0]; //lấy sau thằng image vì nó qua folder name là image
      ...
      const fileNamevideo = `${folderName}/${Date.now()}-${newFile.originalname}`;
